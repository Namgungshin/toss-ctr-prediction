...
| `submissions/blend_base70_seq30.csv` | n/a | 미제출 | 베이스라인 70% + seq 30% 보수적 블렌드 후보 |
...
- 기본 `seq` 피처는 로컬 AP를 조금 올렸지만 WLL은 살짝 나빠졌다. 제출 제한 때문에 리더보드 검증은 다음 제출 가능일에 진행한다.
- 다음 우선순위는 안정적인 범주형 그룹에 대한 빈도 인코딩과 타깃 인코딩이다.
